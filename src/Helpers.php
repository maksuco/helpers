...
function location($ip) {
  $query = unserialize(file_get_contents('http://ip-api.com/php/'.$ip));
    ...
    //lang
    if(in_array($query['countryCode'], ["ES","AR","BO","EC","CW","CO","CR","CU","DO","PR","CL","PA","VE","UY","GT","SV"])) {
      $lang = "ES";
    } elseif(in_array($query['countryCode'], ["US","EN","UK","CA"])) {
      $lang = "EN";
    } elseif(in_array($query['countryCode'], ["PT","BR"])) {
      $lang = "PT";
    } else {
      $lang = false;
    }
    $data = ['city' => $query['city'], 'country' => $query['countryCode'], 'timezone' => $query['timezone'], 'continent' => $COUNTRY_CONTINENTS[$query['countryCode']], 'lang' => $lang];
    return $data;
  } else {
    return false;
  }
