...
class Helpers
{
    ...
    public function user_agent($user_agent = null)
    {
        ...
        if (strpos($user_agent, 'Opera') || strpos($user_agent, 'OPR/')) {
            $data['browser'] = 'Opera';
        } elseif (strpos($user_agent, 'Googlebot')) {
            $data['browser'] = 'Googlebot';
        ...
        if (strpos($user_agent, 'Macintosh') !== false) {
            $data['os'] = 'Mac';
        } elseif (strpos($user_agent, 'iPhone') !== false || strpos($user_agent, 'iOS') !== false) {
            $data['os'] = 'ios';
    ...
    // 	echo $key . " ";
    // }
    public function gFonts($url, $config = [])
    {
    ...
    public function botDetected($user_agent = false)
    {
        $user_agent = $user_agent ?? $_SERVER['HTTP_USER_AGENT'] ?? false;
        if ($user_agent != false && preg_match('/bot|crawl|slurp|spider|curl|mediapartners/i', $user_agent)) {
            return true;
        }
    ...
    // BROWSER LOCALE
    public function browserLocale($languages)
    {
    ...
    public function nav_active($page)
    {
        if (isset($active)) {
    ...
    public function text_parse($text, $blank = true)
    {
    ...
    public function replaceTextBetween($text, $start = '', $end = '', $replace = false)
    {
        $search = '#('.$start.').*?('.$end.')#';
    ...
    public function anyNumberFormat($value, $currency = null)
    {
    ...
    public function moneyFormat($value = null, $currency = 'USD')
    {
        if ($value == null) {
            return '';
        }
    ...
    public function decimalsFormat($number, $cents = true)
    {
    ...
    public function geoip($ip, $optional = 'city', $key = null)
    {
        if ($ip == null) {
    ...
    public function geoipLaravel($ip, $optional = 'city')
    {
        if ($ip == null) {
            $ip = (isset($_SERVER['HTTP_CF_CONNECTING_IP'])) ? $_SERVER['HTTP_CF_CONNECTING_IP'] : $_SERVER['REMOTE_ADDR'];
        }
    ...
    public function extractFilesFromJson($jsonString)
    {
    ...
    public function appendtojson($json, $new, $subcategory = false, $limit = false)
    {
    ...
    public function csvstring($action, $data, $new)
    {
        $data = explode(',', $data);
        $exist = false;
    ...
    public function array_process($action, $array, $new)
    {
        // return $array[$new];
        $exist = false;
    ...
    public function collection_relation($principal, $second, $second_relation_column, $values, $principal_relation_column = 'id')
    {
    ...
    public function column_check($data, $value)
    {
    ...
    public function column_process($data, $table, $column, $value)
    {
        include_once 'Extras/column.php';
    ...
    public function countries($lang = 'en')
    {
        if ($lang == 'es') {
    ...
    public function country($isoCode = 'US')
    {
   ...
   public function citiesx($countryCode = 'US', $city = false, $persist = false)
   {
       return [];
   ...
   private function fetchCityFromApi($city, $countryCode)
   {
    ...
    public function languages($lang = 'all', $laravel = false)
    {
        if (class_exists("Illuminate\Foundation\Application")) {
    ...
    public function alternateLang($lang, $langs = false)
    {
        if (! $langs) {
            return $lang === 'en' ? 'es' : 'en';
        }

        return array_filter($langs, function ($row) {
            return $row !== $currentLang;
        });
    }

    public function altSlug($slugs, $lang, $principal = 'en')
    {
    ...
    public function currencies($isoCode = null)
    {
    ...
    public function currency_format($amount, $isoCode, $array = false)
    {
        include __DIR__.'/Extras/currencies.php';
    ...
    public function currency_exchange($amount, $from, $to = 'usd', $round = false)
    {
    ...
    public function country_continents($countryCode)
    {
    ...
    public function icon($name, $class = false, $extras = '')
    {
    ...
    public function icons($name, $action = 'all')
    {
    ...
    // GetAvatar
    public function avatar($user, $s3 = null)
    {
    ...
    public function lettersToNumbers($string, $phone = false)
    {
    ...
    public function sizetobytes($size, $reverse = false)
    {
        // UNITS TO BYTES
        if (! $reverse) {
    ...
    public function slug_file($filename, $random)
    {
    ...
    public function slug_filename($filename, $name, $random)
    {
    ...
    public function slug_username($string)
    {
        $replace = $this->slugReplace();
    ...
    public function slug_random($characters)
    {
    ...
    public function random($characters = 1, $numbers = null)
    {
        if ($numbers == null) {
    ...
    // Because str_random doesn't work anymore
    public function random_name($type = null)
    {
    ...
    public function hide_string($string, $middle = '****', $position = false)
    {
        if ($position == 'start') {
    ...
    // Because str_random doesn't work anymore
    public function greetings_by_time($timezone = false)
    {
        if ($timezone) {
    ...
    public function date_day($date_or_day, $lang = 'en')
    {
    ...
    public function getVideoID($provider, $string)
    {
    ...
    public function domainName($link)
    {
        $link = $this->link($link);
    ...
    public function domainPrimary($link)
    {
        $host = parse_url($link, PHP_URL_HOST);
    ...
    public function url_html($url, $section = 'body', $replace_url = false)
    {
    ...
    public function VeeValidateHoneypot()
    {
        $random = rand();
...
HTML;
    ...
    public function VeeValidateHoneypotCSS()
    {
        $random = rand();
...
HTML;
    ...
    public function period($period, $period2)
    {
        if ($period == 'month') {
    ...
    public function counter($period, $period2, $path)
    {
    ...
    public function linkedinshare($url, $title, $username = null)
    {
        $url = ($url) ? $url : (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http')."://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
    ...
    public function whatsappchat($phone, $url, $text = false)
    {
        $text = ($text) ? $text.' ' : '';
    ...
    {
        $month = $data->filter(function ($user) {
            return $user->created_at->format('m-y') == date('m-y');
        });
        $lastmonth = $data->filter(function ($user) {
            return $user->created_at->format('m-y') == Carbon::now()->firstOfMonth()->subMonth()->format('m-y');
        });
        $year = $data->filter(function ($user) {
            return $user->created_at->format('y') == date('y');
        });
        $lastyear = $data->filter(function ($user) {
            return $user->created_at->format('y') == Carbon::now()->subYear()->format('y');
        });
    ...
    public function chart($period, $period2, $path)
    {
    ...
    public function charts($data1)
    {
        $startDate = Carbon::now()->subDay(365);
        $endDate = Carbon::now();

        // THIS RETURNS THE 4 BOXES
        $month = $data->filter(function ($data1) {
            return $data1->created_at->format('m-y') == date('m-y');
        });
        $lastmonth = $data->filter(function ($data1) {
            return $data1->created_at->format('m-y') == Carbon::now()->firstOfMonth()->subMonth()->format('m-y');
        });
        $year = $data->filter(function ($data1) {
            return $data1->created_at->format('y') == date('y');
        });
        $lastyear = $data->filter(function ($data1) {
            return $data1->created_at->format('y') == Carbon::now()->subYear()->format('y');
        });

    }

    // I think is better to cretae a reports_total and divide this and also implement in invoices.blade
    public function reports($biz_id, $table, $date, $sum)
    {
    ...
    public function reports_chart($resultsDB, $date, $sum, $sum2)
    {
    ...
    public function instagram_process($instagram, $username)
    {
    ...
    public function getTikTok($username,$type = 'profile')
    {
        if ($type == 'profile') {
